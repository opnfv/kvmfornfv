...
debbuild_tmp='/tmp'
debbuild_dir=$debbuild_tmp/kvmfornfv_debbuild.$$
echo $debbuild_dir
mkdir -p $debbuild_dir
artifact_debs=$debbuild_dir/DEBS
mkdir -p $artifact_debs
artifact_dir=$artifact_debs/x86_64
...
( cd ${kernel_src_dir}; make DEBOPTS="--define '_topdir ${debbuild_dir}'" deb-pkg )
if [ ${?} -ne 0 ] ; then
