...
function run_qemusrc() {
  $qemu -enable-kvm -cpu host -smp ${guest_cpus} -chardev socket,id=char1,path=$VHOSTPATH1 \
        ...
        -drive file=/root/guest1.qcow2 -vnc :1 &
  if [ ${?} -ne 0 ] ; then
...
function run_qemulisten() {
  ...
  -drive file=/root/guest1.qcow2 -incoming tcp:${incoming_ip}:${migrate_port} -vnc :3 &
  if [ ${?} -ne 0 ] ; then
...
run_qemulisten
if [ ! -f "$qmp_sock_src" ];then
   echo "qmp sock file not created"
   exit 1
else
   sleep 30
