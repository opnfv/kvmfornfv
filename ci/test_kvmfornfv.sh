...
function cyclictest {
   ...
   sudo scp root@${HOST_IP}:/root/MBWInfo/MBWInfo* $WORKSPACE/build_output/log/MBWInfo/
   ls $WORKSPACE/build_output/log/MBWInfo/
}
function collect_MBWInfo {
   ...
   sudo ssh root@${HOST_IP} "${pcm_memory} 60 > /root/MBWInfo/MBWInfo_${testType}_${timeStamp}"
}
