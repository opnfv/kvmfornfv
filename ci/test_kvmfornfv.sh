...
function ftrace_disable { 
   ...
   sudo ssh root@${HOST_IP} "mkdir -p $WORKSPACE/build_output/log/kernel_trace"
   sudo ssh root@${HOST_IP} "mv /tmp/cyclictest_${env}.txt $WORKSPACE/build_output/log/kernel_trace"
   sudo ssh root@${HOST_IP} "cd $WORKSPACE/build_output/log ; tar -czvf kernel_trace-$1.tar.gz kernel_trace"
}
...
if [ ${test_type} == "verify" ];then
   ...
   if [ ${ftrace_enable} -eq '1' ]; then
      ...
      do
         ...
         ftrace_disable $GERRIT_CHANGE_NUMBER
         sleep 10
      done
   ...
   if [ ${test_name} == "packet_forward" ];then
      ...
      if [ ${ftrace_enable} -eq '1' ]; then
         ...
         do
            ...
            cyclictest ${env}
            #disabling ftrace and collecting the logs to upload to artifact repository. 
            ftrace_disable $(date -u +"%Y-%m-%d_%T")
            sleep 5
         done
      else
         for env in ${cyclictest_env_daily[@]}
